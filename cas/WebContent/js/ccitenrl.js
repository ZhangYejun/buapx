...
//{
//	cabpath = "codebase=\"" + rootpath + "/conf/ccitenrl.cab#version=" + cabversion + "\"";
...
function restoreKey(packdata, transactionId)
{
    try{
	 ...
	 encCert = pack.substring(start, end);
        ...
        UniContrl.keySpec = KT_EXCHANGE;
        UniContrl.saveKeyPairEx2(UniContrl.GetCertPubKey(encCert), encPriKey, KET_ENCBYSIGNKEY);
...
//新添接口，为兼容签名服务器使用。pangsiming2010-12-21
//初始化：Type==1实名证书
//				Type==0匿名证书
//		返回0：成功
//		返回其他：失败
...
try{
		var ret; 
		if(Type==1)//实名证书
  		{ret = UniContrl.init("PROVIDE1","","");}
  		else
  			{ret = UniContrl.init("PROVIDE","","");}
...
function ClearUserPin()
{
	try{
		var ret; 
  		...
  		ret = UniContrl.ClearUserPinAndCache();
